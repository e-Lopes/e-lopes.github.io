...
const sourcePath = path.join(root, 'docs', 'guides', 'guia-administradores-digistats-v1.2.md');
const outputPath = path.join(root, 'docs', 'guides', 'guia-administradores-digistats-v1.2.html');
const pdfPath = path.join(root, 'docs', 'guides', 'Guia-do-Administrador-DigiStats-v1.2.pdf');
...
<title>Guia do Administrador — DigiStats — Versão 1.2</title>
<style>
...
if (!edgePath) {
    ...
    const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'digistats-admin-guide-v1-2-'));
    ...
    if (result.status !== 0) {
        console.error('Não foi possível gerar o PDF do guia administrativo v1.2.');
